...
exports.createNewBrand = async (brandName, sessionId) => {
    ...
    let response = await apiCallService.post(uri + '/brands/create', data, sessionId)
...
exports.getBrandList = async (sessionId) => {
    let response = await apiCallService.get(uri + '/brands/list', sessionId);
    if (response.getDataError !== null) {
        return {
            'brandList': null,
            'isForbidden': response.isLoginError,
...
exports.getBrandById = async (brandId, sessionId) => {
    let response = await apiCallService.get(uri + '/brands/' + brandId, sessionId);
    if (response.getDataError !== null) {
        return {
            'brand': null,
            'isForbidden': response.isLoginError,
...
exports.updateBrandById = async (brandId, brandName, sessionId) => {
    ...
    let response = await apiCallService.post(uri + '/brands/' + brandId, data, sessionId);
    if (response.getDataError !== null) {
        return {
            'brand': null,
            'isForbidden': response.isLoginError,
